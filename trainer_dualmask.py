...
#             p1 = torch.argmax(outputs_soft1[labeled_bs:].detach(), dim=1, keepdim=False)
#             p2 = torch.argmax(outputs_soft2[labeled_bs:].detach(), dim=1, keepdim=False)
#             if iter_num < args.warm_iter:
#                 consistency_loss=0.0
#             else:
#                 consistency_loss2 = 0.5 * (ce_loss(outputs2[labeled_bs:], p1.long()) + dice_loss(
#                     outputs2[labeled_bs:], p1, softmax=True))
#                 consistency_loss1 = 0.5 * (ce_loss(outputs1[labeled_bs:], p2.long()) + dice_loss(
#                 outputs1[labeled_bs:], p2, softmax=True))
#                 consistency_loss=consistency_weight*(consistency_loss1+consistency_loss2)
#
#             loss1=loss1+consistency_loss
#             optimizer.zero_grad()
#             loss1.backward()
#             optimizer.step()
#             # # the second round
...
import torch.nn.functional as F
from tqdm import tqdm
from utils import DiceLoss, sigmoid_rampup, calculate_score,  custom_mask
from torchvision import transforms
from icecream import ic
from val import test_single_volume
from datasets.dataset_ACDC import TwoStreamBatchSampler
