...
class Sam_dualmask_same_prompt_class_random_large(nn.Module):
    ...
    def forward_train(self, batched_input, multimask_output, image_size, prompt_idx, prompt):
        ...
        if prompt_idx == 0:
            ...
            points_prompt, points_prompt_random, box_prompt, mask_prompt = self.prompt_generate_random_fast(
                low_res_masks1, image_size, True)
            ...
            if prompt == 'point':
                ...
                sparse_embeddings_r, _ = self.prompt_encoder(
                    points=points_prompt_random, boxes=None, masks=mask_prompt
                )
            elif prompt == 'box':
                sparse_embeddings, dense_embeddings = self.prompt_encoder(
            ...
            # generate prompts based on the coarse prediction
            points_prompt, points_prompt_random, box_prompt, mask_prompt = self.prompt_generate_random_fast(
                low_res_masks2, image_size, True)
    ...
    #     coarse_mask_np = F.interpolate(coarse_mask_np.unsqueeze(1).float(), (img_size, img_size),
    ...
    #                     points_prompt_random[idx, cls, 0], points_prompt_random[idx, cls, 1] = int(
    #                         cX_r[random_idx]), int(cY_r[random_idx])
    #
    ...
    #                 dist_img = cv2.distanceTransform(padded_mask, distanceType=cv2.DIST_L2, maskSize=5).astype(
    #                     np.float32)[1:-1, 1:-1]
    ...
    def prompt_generate_random_fast(self, coarse_mask, img_size, israndom = False):  # generate point prompts
        ...
        coarse_mask_np = torch.argmax(coarse_mask, dim=1)
        mask_prompt = coarse_mask_np.unsqueeze(1)
        pool = nn.MaxPool2d(kernel_size=4, stride=4)
        mask_prompt = pool(mask_prompt.float())
        ...
        if israndom:
            ...
            return None, None, None, mask_prompt

        return None, None, mask_prompt